...
const getOneMap = async (id , category) => {
  if (category === 'about' || category === '3d') {
    try {
...
<template>
  <main :class="store.slidOpen === true ? 'right-0' : 'right-[-20rem]'" class="duration-200 z-50 fixed justify-center w-80 right-0 top-16 h-full bg-white p-5">
    ...
    <div @click="getOneMap(data.id, data.category)" v-for="data in store.slidData" class="w-full cursor-pointer gap-2 flex overflow-hidden justify-center p-2.5 border border-secondary-100 hover:border-secondary-200 hover:scale-105 duration-150 rounded-md bg-secondary-50 mb-1.5">
      ...
      <div v-else-if="data.category === 'about'" class="size-4 p-1 bg-purple-600 mt-0.5 rounded-full"></div>
      <div v-else-if="data.category === '3d'" class="size-4 p-1 bg-pink-600 mt-0.5 rounded-full"></div>
    </div>
